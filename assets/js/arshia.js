...
$window.on("popstate", function () {
    ...
    if ($lastHash == 1) {
        ...
        if (($id == $last && !($thisId == $first)) || ($id == $first && !($thisId == $last))) {
            openMenu();
            if ($window.width() < 992) {
        ...
        } else if (($id == $last && $thisId == $first) || ($id == $first && $thisId == $last)) {
            $("#main > section.active").addClass(func[1]).removeClass("active");
            $main.children($id).addClass('active ' + func[0]);
        } else if (($id != $last && $thisId != $first) || ($id != $first && $thisId != $last)) {
            $("#main > section.active").addClass(func[1]).removeClass("active");
...
function skills() {
    "use strict";
    var hT = $('.skills').offset().top, hH = $('.skills').outerHeight(), wH = $(window).height(), wS = $(window).scrollTop(), percent, progressEnd, skillDP;
    if (wS > (hT + hH - wH)) {
...
function animateRandom() {
    const animate = [["animate__backInDown", "animate__backOutDown"], ["animate__zoomIn", "animate__zoomOut"], ["animate__fadeInDown", "animate__fadeOutDown"],];
